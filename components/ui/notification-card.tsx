...
import { enUS } from 'date-fns/locale' // Changed from `fr` to `enUS`
...
export function NotificationCard({
    ...
    const handleMarkAsUnRead = async () => {
        ...
        try {
            ...
            if (result.success) {
                onMarkAsUnRead?.()  // You can replace this function with onMarkAsUnread if you want separate handling
    ...
    return (
        ...
        >
            ...
            <div className="flex-1 min-w-0 pr-12">
                ...
                <p className="text-xs text-gray-500 mt-1">
                    {formatDistanceToNow(createdAt, {
                        addSuffix: true,
                        locale: enUS // Changed to `enUS` for English
                    })}
                </p>
                {showWorkspace && (
            ...
            {/* Buttons only shown on hover over the main div */}
            <div
                className={`
                ...
                `}
            >
                {/* The Bell icon changes based on the isRead state */}
                <Button
                    variant="ghost"
                    size="icon"
