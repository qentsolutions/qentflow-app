...
export const ServerSearch = ({
  ...
  const onClick = ({ id, type }: { id: string, type: "channel" | "member" }) => {
    ...
    if (type === "member") {
      return router.push(`/${currentWorkspace?.id}/servers/${params?.serverId}/conversations/${id}`)
    }

    if (type === "channel") {
      return router.push(`/${currentWorkspace?.id}/servers/${params?.serverId}/channels/${id}`)
    }
  }
